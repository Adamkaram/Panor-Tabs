    ...
    document.getElementById('content').value = '';
      for (var i = 0; i < numWindows; i++) {
       ...
       var chld =  document.getElementById('content').value += tab.title + '\n';
        var spn = document.createElement('span');
        spn.style.color = "red";
        txt = document.createTextNode(chld);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // document.querySelector('#MN').addEventListener('click', ChangeColorMode);
  document.querySelector('#sendMail0').addEventListener('click', function(){sendMail(0)});
  document.querySelector('#cut').addEventListener('click', function(){sendMail(1)});
